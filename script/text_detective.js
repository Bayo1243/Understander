...
// Tab Elements
// (These are selected in main.js now)
...
/**
 * Generates the HTML for the redacted text based on difficulty.
 * @param {string} baseHTML - The pre-formatted HTML with <strong> and <code> tags.
 * @param {string} difficulty - 'beginner', 'moderate', 'advanced', or 'expert'.
 * @returns {string} - The new HTML with some <code> tags converted to <span>.
 */
function generateRedactedHTML(baseHTML, difficulty) {
    const redactionRates = {
        beginner: 0.7, // Show 70% of code blocks
        moderate: 0.4, // Show 40%
        advanced: 0.1, // Show 10%
        expert: 0.0     // Show 0%
    };
    const showRate = redactionRates[difficulty] || 0.4; // Default to moderate
    ...
    codeBlocks.forEach(code => {
        // Decide whether to un-redact this block
        if (Math.random() < showRate) {
            // Replace <code>...</code> with <span class="un-redacted">...</span>
            const span = document.createElement('span');
            span.className = 'un-redacted';
            span.innerHTML = code.innerHTML; // Transfer content
 ...
 * @param {number} index - The index of the challenge in textPairs.
...
function setupDetectiveChallenge(index, difficulty) {
    ...
    currentDifficulty = difficulty; // Store the difficulty for this session
    isRevealed = false; 
    quizSubmitted = false;
...
/**
 * Populates the quiz container with randomized options for the given challenge index.
 * @param {number} index - The index of the challenge in textPairs.
 */
...
function handleQuizSubmit() {
    ...
    // Show "Next Challenge" and "Reveal" buttons
...
/**
 * Toggles the text between its full and redacted state (Guided Mode).
 */
